// packages/host/src/transfer-worker.js
...
//
// Mirrors the controller's packages/controller/src/transfer-worker.js
// verbatim (same IPC-topic/window-security contract) — the host side is the
// RECEIVE half of the flagship flow: main.js only ever calls
// startRendezvous({role:'attach', ...}) here (a peer's transfer-worker
// initiates; this host attaches by the sessionId relayed via TRANSFER_REQUEST).
// Multiple workers may exist at once, so every IPC topic is namespaced per
// worker via a unique workerId — two workers can never cross streams.
...
export function createTransferWorker() {
  ...
  return {
    ...
    // design §4.2/§4.3 for the two rendezvous shapes. The host only ever uses
