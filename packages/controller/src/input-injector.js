// packages/controller/src/input-injector.js
// Verbose diagnostic logging (see docs/private/superpowers): counts/shape-level
// facts only — never key values or coordinates.
...
export function createInjector({ nut, display, dipToScreen = (p) => p, log = noopLog() }) {
  ...
  const enqueue = (fn) => {
    chain = chain.then(fn).catch((e) => { log.error(`injection failed: ${e?.message ?? e}`); });
    return chain;
  };
  return {
    setDisplay(next) {
      region = next.bounds;
      log.info(`injector display -> ${next.index}`);
    },
    inject(rawEvent) {
      let evt;
      try { evt = validateInputEvent(rawEvent); } catch { log.warn('dropped invalid input event'); return false; }
      switch (evt.type) {
